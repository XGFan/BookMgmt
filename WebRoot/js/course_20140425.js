...
$(function() {
	...
	function loadCourseData(data,target) {
		...
		if (data.length >= 1) {
			strHTML += "<tr align='center'><th>课程编号</th><th>学院</th><th>专业</th><th>课程名称</th><th>学期</th><th>查看</th><th>编辑</th><th>删除</th></tr>";
			$.each(data,function(index, Info) {
				...
				strHTML += "<td width='22%' align='center'>" + Info["major"]
						+ "</td>";
				strHTML += "<td width='28%' align='center'>" + Info["corname"]
						+ "</td>";
	...
	function showCourseInfo(col,major,idcor,corname,semester){
		var strHTML = "<p align='left'><font size='5' color='#0000ff'>"+col+","+major+"专业，第"+semester+"学期的课程<br/> '"+corname+"' 选用教材如下：</font>";
		$.ajax({
			type : "post",
			dataType : "json",
			url : "course_findByCourse.action",
...
//								strHTML += "<tr class='trOdd'><td width='5%' align='center'>"+Info["idbk"]+"</td>";
	...
	function deleteCourse(idcor,corname,semester) {
		$.ajax({
			type : "post",
			dataType : "json",
	...
	$("#deletebtn").live("click", function() {
		...
		deleteCourse(idcor,corname,semester);
	});
	...
	$("#viewbtn").live("click", function(){
		...
		//alert(col+","+major+","+corname+","+idcor+","+semester);
